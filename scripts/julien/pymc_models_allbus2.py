...
def create_allbus_tabu_aiccg(filename="", modelname="allbus_tabu_aiccg", fit=True):
    ...
    with model:
        ...
        lived_abroad = pm.Categorical('lived_abroad', p=[0.8145, 0.1855])
        age = pm.Normal('age', mu=tt.switch(tt.eq(lived_abroad, 0), 53.1309, 48.9693),
                        sigma=tt.switch(tt.eq(lived_abroad, 0), 17.3465, 17.4244))
        educ = pm.Normal('educ', mu=tt.switch(tt.eq(lived_abroad, 0),
                                              tt.switch(tt.eq(sex, 0), age * -0.0245 + 4.6575, age * -0.0174 + 4.2596),
                                              tt.switch(tt.eq(sex, 0), age * -0.0249 + 5.3784, age * -0.0074 + 4.2735)),
                         sigma=tt.switch(tt.eq(lived_abroad, 0), tt.switch(tt.eq(sex, 0), 1.0465, 1.1167),
                                         tt.switch(tt.eq(sex, 0), 1.0003, 1.2143)))
        eastwest = pm.Normal('eastwest',
                             mu=tt.switch(tt.eq(lived_abroad, 0), age * -0.0026 + 0.7671, age * -0.0002 + 0.8002),
                             sigma=tt.switch(tt.eq(lived_abroad, 0), 0.4808, 0.4085))
        income = pm.Normal('income', mu=tt.switch(tt.eq(lived_abroad, 0), tt.switch(tt.eq(sex, 0),
                                                                                    age * 6.6264 + educ * 258.9081 + eastwest * 112.1817 + -6.3859,
                                                                                    age * 11.2241 + educ * 379.6058 + eastwest * 669.0161 + -246.3414),
                                                  tt.switch(tt.eq(sex, 0),
                                                            age * 5.4384 + educ * 212.5717 + eastwest * 17.0135 + 378.0212,
                                                            age * 10.8018 + educ * 423.9158 + eastwest * 731.6931 + -290.5039)),
                           sigma=tt.switch(tt.eq(lived_abroad, 0), tt.switch(tt.eq(sex, 0), 736.4412, 1198.2071),
                                           tt.switch(tt.eq(sex, 0), 978.8413, 1578.9939)))
        health = pm.Normal('health', mu=tt.switch(tt.eq(sex, 0),
                                                  age * -0.0133 + educ * 0.1332 + income * 0.0001 + eastwest * 0.1897 + 3.6014,
                                                  age * -0.0182 + educ * 0.1163 + income * 0.0001 + eastwest * -0.0397 + 3.9413),
                           sigma=tt.switch(tt.eq(sex, 0), 0.9475, 0.8755))
        happiness = pm.Normal('happiness', mu=tt.switch(tt.eq(sex, 0),
                                                        age * 0.0203 + educ * 0.1757 + income * 0.0 + eastwest * 0.3886 + health * 0.7865 + 3.0787,
                                                        age * 0.0103 + educ * -0.0054 + income * 0.0002 + eastwest * 0.3731 + health * 0.6639 + 4.1456),
                              sigma=tt.switch(tt.eq(sex, 0), 1.5127, 1.5539))
...
def create_allbus_tabu_biccg(filename="", modelname="allbus_tabu_biccg", fit=True):
    ...
    with model:
        sex = pm.Categorical('sex', p=[0.4803, 0.5197])
        lived_abroad = pm.Categorical('lived_abroad', p=[0.8145, 0.1855])
        educ = pm.Normal('educ', mu=tt.switch(tt.eq(lived_abroad, 0), 3.3446, 4.0426),
                         sigma=tt.switch(tt.eq(lived_abroad, 0), 1.1427, 1.1696))
        eastwest = pm.Normal('eastwest', mu=tt.switch(tt.eq(lived_abroad, 0), 0.6306, 0.7896),
                             sigma=tt.switch(tt.eq(lived_abroad, 0), 0.4828, 0.4081))
        ...
        income = pm.Normal('income', mu=tt.switch(tt.eq(sex, 0),
                                                  educ * 211.6522 + eastwest * 64.2077 + happiness * 31.6485 + 294.2901,
                                                  educ * 330.5615 + eastwest * 586.4313 + happiness * 160.9824 + -645.3277),
                           sigma=tt.switch(tt.eq(sex, 0), 787.7015, 1264.8692))
        age = pm.Normal('age',
                        mu=educ * -4.2074 + income * 0.0021 + eastwest * -3.5511 + happiness * 1.53 + health * -5.7348 + 74.4703,
                        sigma=15.534)
...
def create_allbus_hc_loglikcg(filename="", modelname="allbus_hc_loglikcg", fit=True):
    ...
    with model:
        ...
        age = pm.Normal('age', mu=tt.switch(tt.eq(lived_abroad, 0), tt.switch(tt.eq(sex, 0), 52.9369, 53.3145),
                                            tt.switch(tt.eq(sex, 0), 46.6042, 50.9351)),
                        sigma=tt.switch(tt.eq(lived_abroad, 0), tt.switch(tt.eq(sex, 0), 17.2198, 17.4726),
                                        tt.switch(tt.eq(sex, 0), 17.1287, 17.4603)))
        eastwest = pm.Normal('eastwest', mu=tt.switch(tt.eq(lived_abroad, 0),
                                                      tt.switch(tt.eq(sex, 0), age * -0.0032 + 0.8152,
                                                                age * -0.002 + 0.7215),
                                                      tt.switch(tt.eq(sex, 0), age * -0.0016 + 0.8778,
                                                                age * 0.0011 + 0.7257)),
                             sigma=tt.switch(tt.eq(lived_abroad, 0), tt.switch(tt.eq(sex, 0), 0.476, 0.4853),
                                             tt.switch(tt.eq(sex, 0), 0.3995, 0.4162)))
        happiness = pm.Normal('happiness', mu=tt.switch(tt.eq(lived_abroad, 0), tt.switch(tt.eq(sex, 0),
                                                                                          age * 0.0012 + eastwest * 0.5048 + 7.4822,
                                                                                          age * -0.0013 + eastwest * 0.5671 + 7.409),
                                                        tt.switch(tt.eq(sex, 0),
                                                                  age * 0.0114 + eastwest * 0.6352 + 7.1522,
                                                                  age * -0.0018 + eastwest * 0.3592 + 7.7156)),
                              sigma=tt.switch(tt.eq(lived_abroad, 0), tt.switch(tt.eq(sex, 0), 1.7537, 1.7176),
                                              tt.switch(tt.eq(sex, 0), 1.5355, 1.6763)))
        health = pm.Normal('health', mu=tt.switch(tt.eq(lived_abroad, 0), tt.switch(tt.eq(sex, 0),
                                                                                    age * -0.0173 + eastwest * 0.0619 + happiness * 0.2475 + 2.5132,
                                                                                    age * -0.0196 + eastwest * -0.0613 + happiness * 0.1902 + 3.1754),
                                                  tt.switch(tt.eq(sex, 0),
                                                            age * -0.017 + eastwest * -0.0689 + happiness * 0.3268 + 2.0766,
                                                            age * -0.0169 + eastwest * -0.1616 + happiness * 0.2657 + 2.6144)),
                           sigma=tt.switch(tt.eq(lived_abroad, 0), tt.switch(tt.eq(sex, 0), 0.8605, 0.8195),
                                           tt.switch(tt.eq(sex, 0), 0.8248, 0.8931)))
        educ = pm.Normal('educ', mu=tt.switch(tt.eq(lived_abroad, 0), tt.switch(tt.eq(sex, 0),
                                                                                age * -0.0234 + eastwest * -0.1437 + happiness * 0.0975 + health * 0.0867 + 3.6135,
                                                                                age * -0.0132 + eastwest * -0.0496 + happiness * 0.0388 + health * 0.2095 + 3.0245),
                                              tt.switch(tt.eq(sex, 0),
                                                        age * -0.022 + eastwest * -0.0631 + happiness * 0.0012 + health * 0.2225 + 4.4149,
                                                        age * -0.0025 + eastwest * -0.0319 + happiness * 0.0166 + health * 0.276 + 2.8903)),
                         sigma=tt.switch(tt.eq(lived_abroad, 0), tt.switch(tt.eq(sex, 0), 1.0239, 1.0968),
                                         tt.switch(tt.eq(sex, 0), 0.9847, 1.1873)))
        income = pm.Normal('income', mu=tt.switch(tt.eq(lived_abroad, 0), tt.switch(tt.eq(sex, 0),
                                                                                    age * 7.698 + educ * 246.3672 + eastwest * 94.4126 + happiness * 3.1363 + health * 81.3826 + -325.8667,
                                                                                    age * 13.3169 + educ * 339.5376 + eastwest * 600.2035 + happiness * 109.0891 + health * 133.7577 + -1496.6301),
                                                  tt.switch(tt.eq(sex, 0),
                                                            age * 3.8995 + educ * 214.9521 + eastwest * -14.5387 + happiness * 64.0641 + health * -65.358 + 195.4509,
                                                            age * 12.7955 + educ * 359.5552 + eastwest * 648.6692 + happiness * 244.7378 + health * 116.8458 + -2446.7166)),
                           sigma=tt.switch(tt.eq(lived_abroad, 0), tt.switch(tt.eq(sex, 0), 732.8992, 1172.662),
                                           tt.switch(tt.eq(sex, 0), 980.3736, 1514.6133)))
...
def create_allbus_hc_aiccg(filename="", modelname="allbus_hc_aiccg", fit=True):
    ...
    with model:
        ...
        eastwest = pm.Normal('eastwest', mu=tt.switch(tt.eq(lived_abroad, 0), 0.6306, 0.7896),
                             sigma=tt.switch(tt.eq(lived_abroad, 0), 0.4828, 0.4081))
        age = pm.Normal('age',
                        mu=tt.switch(tt.eq(lived_abroad, 0), eastwest * -3.3178 + 55.223, eastwest * -0.3947 + 49.2809),
                        sigma=tt.switch(tt.eq(lived_abroad, 0), 17.2771, 17.4443))
        educ = pm.Normal('educ', mu=tt.switch(tt.eq(lived_abroad, 0),
                                              tt.switch(tt.eq(sex, 0), age * -0.0245 + 4.6575, age * -0.0174 + 4.2596),
                                              tt.switch(tt.eq(sex, 0), age * -0.0249 + 5.3784, age * -0.0074 + 4.2735)),
                         sigma=tt.switch(tt.eq(lived_abroad, 0), tt.switch(tt.eq(sex, 0), 1.0465, 1.1167),
        ...
        income = pm.Normal('income', mu=tt.switch(tt.eq(lived_abroad, 0), tt.switch(tt.eq(sex, 0),
                           ...
                           sigma=tt.switch(tt.eq(lived_abroad, 0), tt.switch(tt.eq(sex, 0), 736.4412, 1198.2071),
                                           tt.switch(tt.eq(sex, 0), 978.8413, 1578.9939)))
        happiness = pm.Normal('happiness', mu=tt.switch(tt.eq(sex, 0),
                                                        age * 0.0098 + educ * 0.2804 + income * 0.0001 + eastwest * 0.5379 + 5.9111,
                                                        age * -0.0018 + educ * 0.0718 + income * 0.0003 + eastwest * 0.3468 + 6.7624),
                              sigma=tt.switch(tt.eq(sex, 0), 1.6857, 1.6585))
        health = pm.Normal('health', mu=tt.switch(tt.eq(sex, 0),
                                                  age * -0.0157 + educ * 0.0635 + income * 0.0001 + eastwest * 0.0561 + happiness * 0.2485 + 2.1325,
                                                  age * -0.0179 + educ * 0.103 + income * 0.0001 + eastwest * -0.1038 + happiness * 0.185 + 2.6903),
                           sigma=tt.switch(tt.eq(sex, 0), 0.8503, 0.8203))
...
def create_allbus_hc_biccg(filename="", modelname="allbus_hc_biccg", fit=True):
    ...
    with model:
        ...
        health = pm.Normal('health', mu=age * -0.0166 + educ * 0.0905 + income * 0.0 + happiness * 0.2162 + 2.395,
                           sigma=0.8371)
...
def create_allbus_gs_loglikcg(filename="", modelname="allbus_gs_loglikcg", fit=True):
    ...
    with model:
        ...
        income = pm.Normal('income', mu=tt.switch(tt.eq(lived_abroad, 0), tt.switch(tt.eq(sex, 0),
                                                                                    eastwest * 64.7548 + happiness * 52.5931 + 830.6537,
                                                                                    eastwest * 559.0629 + happiness * 160.6072 + 449.146),
                                                  tt.switch(tt.eq(sex, 0),
                                                            eastwest * -24.8598 + happiness * 57.8588 + 1087.8252,
                                                            eastwest * 618.1169 + happiness * 306.519 + -423.1545)),
                           sigma=tt.switch(tt.eq(lived_abroad, 0), tt.switch(tt.eq(sex, 0), 778.8786, 1242.734),
                                           tt.switch(tt.eq(sex, 0), 995.2346, 1581.8897)))
        educ = pm.Normal('educ', mu=tt.switch(tt.eq(lived_abroad, 0), age * -0.0214 + income * 0.0003 + 4.0042,
                                              age * -0.0175 + income * 0.0002 + 4.538),
                         sigma=tt.switch(tt.eq(lived_abroad, 0), 1.0339, 1.108))
...
def create_allbus_gs_aiccg(filename="", modelname="allbus_gs_aiccg", fit=True):
    ...
    with model:
        ...
        educ = pm.Normal('educ', mu=tt.switch(tt.eq(lived_abroad, 0),
                                              age * -0.0187 + income * 0.0003 + health * 0.1411 + 3.3888,
                                              age * -0.0136 + income * 0.0002 + health * 0.2281 + 3.5243),
                         sigma=tt.switch(tt.eq(lived_abroad, 0), 1.0261, 1.0869))
