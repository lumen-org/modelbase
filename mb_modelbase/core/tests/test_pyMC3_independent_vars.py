...
class Test(unittest.TestCase):
    ...
    def test_prediction_dependent(self):
        ...
        """
        self.assertTrue(len(mymod.predict(mbase.core.base.Aggregation('Y'),
                                          splitby=mbase.core.base.Split('X1', 'equiinterval'))) > 0,
                        'It should be possible to predict a dependent variable conditioned on an independent one')
        self.assertTrue(len(mymod.predict(mbase.core.base.Aggregation('Y'),
                                          splitby=mbase.core.base.Split('alpha', 'equiinterval'))) > 0,
    ...
    def test_prediction_independent(self):
        ...
        """
        self.assertTrue(mymod.predict(mbase.core.base.Aggregation('X1'),
                                      splitby=mbase.core.base.Split('Y', 'equiinterval')).isnull().all()[0],
                        'There should be no predictions for an independent variable conditioned on a dependent one')
        self.assertTrue(mymod.predict(mbase.core.base.Aggregation('X1'),
                                      splitby=mbase.core.base.Split('X2', 'equiinterval')).isnull().all()[0],
                        'There should be no predictions for an independent variable conditioned on a dependent one')
