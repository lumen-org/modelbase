...
class SumNode(Node):
	...
	def evaluate(self, obs=None, values=[]):
		logprobs = self.evaluate_children(obs, False, values)
		return logsumexp(logprobs, axis=1)

	def evaluate_children(self, obs, equal_weight, values):
		logprobs = np.vstack([child.evaluate(obs, values) for child in self.children]).T
		if equal_weight:
			return logprobs
		else:
	...
	def update(self, obs, params, values=[]):
		logprobs = self.evaluate_children(obs, params.equalweight, values)
