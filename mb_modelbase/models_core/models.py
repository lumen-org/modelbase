...
class Model:
    ...
    def hide(self, dims, val=True):
        ...
        Hiding a dimension does not cause any actual change in the model. It simply removes the hidden dimension from any output generated by the model. In combination with setting default values it provides you with a view on a splice of the model, where hidden dimensions are fixed to their default values / subsets. Dimensions without defaults values/subsets can be hidden.
        ...
        However, hiding a dimension with name 'foo' has the following effects on subsequent queries against the model:
          ...
          * predict: Predictions are done on the conditional model on the defaults. The returned tuples do not contain a value for the hidden and defaulting dimensions.
    ...
    def predict(self, predict, where=None, splitby=None, returnbasemodel=False):
        ...
        Hidden dimensions:
            TODO: fix?
            It is not valid to include any hidden dimension in the result table, and such queries will result in a ValueError(). # TODO: raise error
