...
from mb_modelbase.models_core.mspn.mlutils.statistics import gaussianpdf, nplogpoissonpmf, \
    nplogbernoullipmf_fast
from mb_modelbase.models_core.mspn.mlutils.statistics import logbernoullipmf_fast, bernoullipmf
from mb_modelbase.models_core.mspn.mlutils.statistics import logpoissonpmf, poissonpmf
from mb_modelbase.models_core.mspn.mlutils.statistics import gammapdf, betapdf
import numpy as np
import tensorflow as tf
import tensorflow.contrib.distributions as distributions
from mb_modelbase.models_core.mspn.tfspn.piecewise import is_piecewice_linear_pdf, two_staged_sampling_piecewise_linear
...
class ProductNode(Node):
    ...
    def eval(self, data, index=None):
        return np.sum([c.eval(data, index) for c in self.children], axis=0)
...
class SumNode(Node):
    ...
    def eval(self, data, index=None):
        llchildren = np.array([c.eval(data, index) for c in self.children])
