...
class MyTestCase(unittest.TestCase):
    def test_to_string_cols(self):
        ...
        df2 = data_import.to_string_cols(df, inplace=False)
        df3 = data_import.to_string_cols(df, columns=['A'], inplace=False)
        assert_frame_equal(df2, df3)
        data_import.to_string_cols(df, columns=['A'], inplace=True)
