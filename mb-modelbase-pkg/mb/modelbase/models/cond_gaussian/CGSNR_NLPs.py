...
class GMNLP_GAUSS_SNR:
    ...
    def gradient(self, x):
        ...
        g[0: 1, 0: 0 + self.b_size] = (np.add(np.add(np.add(np.multiply(
            np.dot(np.transpose(np.divide(np.dot(self.X, self.w), np.multiply(self.b, self.b))),
                   np.add(self.x, np.divide(np.dot(self.X, self.w), self.b))),
            np.negative(np.divide(self.b, self.n))), np.divide(
            np.dot(np.transpose(np.add(self.x, np.divide(np.dot(self.X, self.w), self.b))),
                   np.add(self.x, np.divide(np.dot(self.X, self.w), self.b))), self.n)),
            np.negative(np.dot(np.transpose(
                np.multiply(np.divide(self.b, self.n),
                            np.add(self.x, np.divide(
                                np.dot(self.X, self.w),
                                self.b)))), np.divide(
                np.dot(self.X, self.w),
                np.multiply(self.b, self.b))))),
            np.divide(-1.0, self.b)))
        g[0: 1, 0 + self.b_size: 0 + self.b_size + self.w_size] = (np.multiply(2.0, np.dot(
            np.transpose(np.divide(np.multiply(np.divide(self.b, self.n), np.add(self.x, np.divide(
                np.dot(self.X, self.w), self.b))), self.b)), self.X)))
        return g
