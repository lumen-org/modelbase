...
# from modelbase.core import pci_graph
...
class Model:
    ...
    def _set_data_mixed(self, df, silently_drop, split_data=True, num_names=None, cat_names=None, **kwargs):
        ...
        if num_names is None or cat_names is None:
            _, cat_names, num_names = data_import.get_columns_by_dtype(df)
        ...
        if split_data:
            self.test_data, self.data = data_import.split_training_test_data(df)
        else:
            self.data = df
            self.test_data = pd.DataFrame(columns=self.data.columns)
        # derive and set fields
        self.fields = data_import.get_discrete_fields(df, self._categoricals) + \
                      data_import.get_numerical_fields(df, self._numericals)
    ...
    def _set_data_continuous(self, df, silently_drop, **kwargs):
        ...
        # split in categorical and numeric columns
        all, categoricals, numericals = data_import.get_columns_by_dtype(df)
        ...
        # derive and set fields
        self.fields = data_import.get_numerical_fields(self.data, numericals)
    ...
    def _set_data_categorical(self, df, silently_drop, **kwargs):
        ...
        # derive and set fields
        self.fields = data_import.get_discrete_fields(self.data, categoricals)
